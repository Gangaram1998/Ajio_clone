...
const userRegistration=async(req,res)=>{
    try{
        ...
        const user= new UserModel({first_name,last_name,email,password:hashedpassword,role})
        await user.save()
        res.status(200).send({
...
const userLogin=async(req,res)=>{
    try{
        ...
        if(!checkUser){
            res.status(200).send({
                status:200,
                message:"User not found!, please Register",err:false
            })
        }
        ...
        let  token=jwt.sign({userId:checkUser._id,role:checkUser.role},process.env.SecretKey)
        res.status(200).send({
